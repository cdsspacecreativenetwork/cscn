...
export function InstructorMentorshipToggle({ instructorId, enabled, disabled }: InstructorMentorshipToggleProps) {
  ...
  return (
    <button
      ...
      className={`relative flex h-[28px] w-[50px] shrink-0 items-center rounded-full p-[3px] transition disabled:cursor-not-allowed disabled:opacity-50 ${
        enabled ? "bg-[#1C4ED1]" : "bg-[#DFE1E7]"
      }`}
      aria-label={enabled ? "Disable mentorship" : "Enable mentorship"}
      title={enabled ? "Disable mentorship" : "Enable mentorship"}
    >
      <span
