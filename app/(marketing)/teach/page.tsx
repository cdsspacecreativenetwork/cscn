...
// Mock Top Instructors Data for Section 5 Spotlight
...
export default function TeachOnCSCNPage() {
  const router = useRouter();
  const { data: session, status } = useSession();
  const [isPending, setIsPending] = useState(false);

  // CTA Click Handler: Bypasses /onboarding/intent and routes directly to /instructor/onboarding
  const handleGetStarted = async () => {
    ...
    try {
      if (status === 'unauthenticated') {
        // Redirect to signup with intent=INSTRUCTOR and callback to instructor onboarding
        router.push('/signup?intent=INSTRUCTOR&callbackUrl=/instructor/onboarding');
        return;
      }

      // If user is authenticated, save intent asynchronously and route STRAIGHT to /instructor/onboarding
  ...
  return (
    <div className="relative w-full min-h-screen bg-background text-[#040B37] font-inter overflow-x-hidden pt-24 pb-16">
      {/* ---------------------------------------------------- */}
      {/* SECTION 1: HERO SECTION (Udemy Exact Spec)           */}
      {/* ---------------------------------------------------- */}
      <section className="relative w-full bg-background overflow-hidden pt-12 md:pt-16 pb-0">
        <div className="w-full max-w-[86rem] mx-auto px-4 sm:px-6 lg:px-12">
          ...
          {/* Desktop & Laptop Layout (md & lg screens - Proportional Height) */}
          <div className="hidden md:grid md:grid-cols-12 items-center min-h-[440px] lg:min-h-[480px]">
            ...
            {/* Right Column Instructor Photo resting directly on bottom edge */}
          ...
          {/* Mobile Layout (Screenshot 3 Spec) */}
          <div className="flex md:hidden flex-col w-full pb-8">
            {/* Top Photo resting on bottom edge */}
      ...
      {/* ---------------------------------------------------- */}
      {/* SECTION 2: SO MANY REASONS TO START (Component)      */}
      {/* ---------------------------------------------------- */}
      <TeachReasonsSection />
      ...
      {/* ---------------------------------------------------- */}
      {/* SECTION 4: HOW TO BEGIN (Udemy Spec Component)       */}
      {/* ---------------------------------------------------- */}
      <HowToBeginSection />

      {/* ---------------------------------------------------- */}
      {/* SECTION 5: FEATURED TOP INSTRUCTORS SPOTLIGHT         */}
      {/* ---------------------------------------------------- */}
      <section className="w-full max-w-[84rem] mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="flex flex-col sm:flex-row items-start sm:items-end justify-between gap-4 mb-12">
          ...
          {/* Show All Button (target="_blank" to /instructors) */}
          <a
            href="/instructors"
            target="_blank"
      ...
      {/* ---------------------------------------------------- */}
      {/* SECTION 6: FINAL CALL TO ACTION (Component)          */}
      {/* ---------------------------------------------------- */}
      <TeachCTASection />
    </div>
  );
