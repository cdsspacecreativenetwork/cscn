          ...
          {visibleQueue.length > 0 ? (
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {visibleQueue.map((item) => (
                <Link
                  key={item.title}
                  href={item.href.replace("/dashboard/admin", "/admin")}
            ...
            {data.courseQualityIssues.length > 0 ? (
              <div className="divide-y divide-[#EEF2FA]">
                {data.courseQualityIssues.map((course) => (
                  <Link key={course.id} href={`/admin/courses/${course.id}`} className="block py-4 first:pt-0 last:pb-0">
                    <div className="flex items-start justify-between gap-4">
          ...
          {visibleActivity.length > 0 ? (
            <div className="divide-y divide-[#EEF2FA]">
              {visibleActivity.map((item) => (
                <Link key={item.id} href={item.href.replace("/dashboard/admin", "/admin")} className="flex items-center justify-between gap-4 py-4 first:pt-0 last:pb-0">
                  <div className="min-w-0">
