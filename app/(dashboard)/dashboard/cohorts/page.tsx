...
export default async function LearnerCohortsPage() {
  ...
  return (
    <main className="mx-auto max-w-[1480px] space-y-6 p-[clamp(16px,2.78vw,48px)] pb-28 font-jakarta">
      <section className="rounded-[20px] border border-[#E3E8F4] bg-white p-6 sm:p-8">
      ...
      {memberships.length ? (
        <section className="grid gap-5 lg:grid-cols-2">
          {memberships.map(({ cohort, status }) => (
            <Link key={cohort.id} href={`/dashboard/cohorts/${cohort.slug}`} className="group rounded-[18px] border border-[#E3E8F4] bg-white p-6 transition hover:-translate-y-0.5 hover:border-[#1C4ED1]/40 hover:shadow-md sm:p-7">
