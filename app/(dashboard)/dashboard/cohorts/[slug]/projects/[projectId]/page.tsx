  ...
  return (
    <main className="mx-auto max-w-[1380px] space-y-6 p-[clamp(16px,2.78vw,48px)] pb-28 font-jakarta">
      ...
      <section className="grid gap-6 xl:grid-cols-[0.8fr_1.2fr]">
        <aside className="space-y-5"><header><p className="text-xs font-semibold tracking-wide text-[#1C4ED1]">{project.cohort.program.title}</p><h1 className="mt-2 text-2xl font-semibold tracking-[-0.03em] text-[#040B37] sm:text-3xl">{project.title}</h1><p className="mt-3 text-sm leading-6 text-[#667085]">{project.brief}</p>{project.dueAt && <p className="mt-3 text-xs font-medium text-[#526078]">Due {project.dueAt.toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })}</p>}</header>
