  ...
  const [cohorts, applications, counts] = await Promise.all([
    ...
    db.cohortApplication.findMany({
      ...
      select: {
        ...
        portfolioUrl: true,
        answers: true,
        user: { select: { id: true, name: true, email: true, profile: { select: { location: true } } } },
        ...
        {selected ? (
          <div className="grid gap-5 lg:grid-cols-[1fr_320px]">
            <article className="rounded-[18px] border border-[#E3E8F4] bg-white p-6">
              <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.14em] text-[#1C4ED1]">{selected.cohort.program.title}</p><h2 className="mt-2 text-2xl font-black text-[#040B37]">{selected.user.name ?? "Unnamed applicant"}</h2><p className="mt-1 text-sm text-[#77839A]">{selected.user.email}{selected.user.profile?.location ? ` · ${selected.user.profile.location}` : ""}</p></div><Link href={`/cohorts/${selected.cohort.slug}`} target="_blank" className="inline-flex items-center gap-2 text-sm font-bold text-[#1C4ED1]">View cohort <ExternalLink size={14} /></Link></div>
